...
var projectKey2Desc = {
   ...
   thesis: 'Coined by Google in 2017, federated learning is a new machine learning paradigm. It allows clients to collaboratively learn a single model from separate datasets of sensitive information. The datasets remain private for the entire duration of the training process.<br><br>The thesis investigated the paradigm\'s robustness to adversarial attacks in both a horizontal and a vertical scenario. Both scenarios\' implementations featured the same model, same datasets, and same number of clients to increase the credibility of their comparison.<br><br>To be finished in August.',
   mp: 'Worked with another student at the University of Zurich to create a system that detects net neutrality breaches. When a user starts the process, the system gathers multiple attributes of their internet connection. These measurements are then placed on the blockchain, available to everyone. Comparisons can be made between the measurements to find discrepancies from net neutrality breaches.<br><br>Responsibilities included creating the a smart contract from scratch, which aggregates, filters, and incentivizes the creation of further user measurements, as well as a GUI to interface with it.<br><br>Read the research paper <a href="resources/MasterProject-Demeter.pdf", download="MasterProject-Demeter.pdf">here</a>.',
